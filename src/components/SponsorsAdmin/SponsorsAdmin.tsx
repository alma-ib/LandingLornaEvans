...
const EMPTY_FORM: FormState = { name: '', url: '', imageFile: null };
...
export const SponsorsAdmin: React.FC = () => {
  ...
  const openEditForm = (item: Sponsor) => {
    setIsCreating(false);
    setEditingItem(item);
    setForm({ name: item.name, url: item.url ?? '', imageFile: null });
    setFormError(null);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.imageFile) {
        imageUrl = await uploadContentImage('sponsors', form.imageFile);
      }

      const input = { name: form.name, imageUrl, url: form.url.trim() || null };
    ...
    <div className="admin-crud-section">
      ...
      {isFormOpen && (
        <form className="admin-crud-form" onSubmit={handleSubmit}>
          ...
          <div>
            <label className="admin-label" htmlFor="sponsor-image">
